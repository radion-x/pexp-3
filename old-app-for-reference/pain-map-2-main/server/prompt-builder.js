...
function generateComprehensivePrompt(formData) {
    ...
    const treatmentGoals = sanitize(formData.treatmentGoals || formData.goals?.goal2to4Weeks || formData.goals?.notes);
...
`;
    ...
    // Pain areas/points
    const painAreas = formData.painAreas || [];
    ...
    if (painAreas.length > 0) {
        painAreas.forEach(area => {
            ...
            const qualities = area.qualities && area.qualities.length > 0 ? area.qualities.join(', ') : '';
        ...
        points.forEach(point => {
            ...
            const qualities = point.qualities && point.qualities.length > 0 ? point.qualities.join(', ') : '';
    ...
    if (formData.timing) {
        ...
        if (t.onset) comprehensivePrompt += `- Onset: ${t.onset.replace(/_/g, ' ')}\n`;
        if (t.durationValue) comprehensivePrompt += `- Duration: ${t.durationValue} ${t.durationUnit || 'months'}\n`;
        if (t.durationMonths !== undefined) {
            ...
            if (months < 1) comprehensivePrompt += `- Duration: Less than 1 month\n`;
                ...
                comprehensivePrompt += `- Duration: ${years} year${years !== 1 ? 's' : ''}${remainingMonths > 0 ? ` and ${remainingMonths} months` : ''}\n`;
            }
        }
        ...
        if (t.timeOfDay && t.timeOfDay.length > 0) {
            comprehensivePrompt += `- Time of Day: ${t.timeOfDay.map(tod => tod.replace(/_/g, ' ')).join(', ')}\n`;
        }
    ...
    if (formData.functional) {
        ...
        comprehensivePrompt += `\nFUNCTIONAL IMPACT:\n`;
        if (func.limits && func.limits.length > 0) {
        ...
        if (func.missedDays7 !== undefined) comprehensivePrompt += `- Missed work/activities (last 7 days): ${func.missedDays7} days\n`;
        if (func.missedDays30 !== undefined) comprehensivePrompt += `- Missed work/activities (last 30 days): ${func.missedDays30} days\n`;
    }
    ...
    if (formData.history) {
        ...
        if (hist.comorbidities && hist.comorbidities.length > 0) {
            comprehensivePrompt += `- Medical conditions: ${hist.comorbidities.join(', ')}\n`;
        }
        if (hist.currentMeds && hist.currentMeds.length > 0) {
            comprehensivePrompt += `- Current medications:\n`;
            hist.currentMeds.forEach(med => {
                ...
                if (med.helpful !== undefined) comprehensivePrompt += ` - ${med.helpful ? 'Helpful' : 'Not helpful'}`;
                if (med.sideEffects) comprehensivePrompt += ` - Side effects: ${sanitize(med.sideEffects)}`;
                comprehensivePrompt += `\n`;
            });
        }
        if (hist.triedTreatments && hist.triedTreatments.length > 0) {
            comprehensivePrompt += `- Prior treatments:\n`;
            hist.triedTreatments.forEach(tx => {
    ...
    if (formData.redFlags) {
        ...
        // Check both old format (boolean flags) and new format (any + reasons)
        if (rf.any) {
            comprehensivePrompt += `⚠️  RED FLAGS PRESENT\n`;
            if (rf.reasons && rf.reasons.length > 0) {
                rf.reasons.forEach(reason => {
                    comprehensivePrompt += `- ${sanitize(reason)}\n`;
                });
            }
        } else {
            // Check individual boolean flags (legacy format)
            const redFlagLabels = {
    ...
    if (formData.goals) {
        ...
        if (goals.preferredTreatments && goals.preferredTreatments.length > 0) {
